...
- Экран конфигурации уведомлений разбивается на **две независимые логические секции** — «Mattermost» и «Email» — каждая со своими полями, **симметричным переключателем включения канала** и собственной кнопкой «Сохранить».
- В секцию Mattermost добавляется переключатель `mattermost_enabled` (симметрично `email_enabled`) — позволяет полностью отключить Mattermost-канал для тенанта, не стирая webhook; при включённом канале без webhook UI показывает мягкое предупреждение.
...
- Бэкенд scheduling: в таблицу `tenant_notification_config` добавляется колонка `mattermost_enabled` (по умолчанию `true`); отдаётся в `GET` и принимается в `PUT`.
- Сервис notification использует новые поля при отправке email: уважает `email_enabled` (выключенный канал пропускается), подставляет `Reply-To` и префикс темы из per-tenant конфигурации. Симметрично уважает `mattermost_enabled`: при `false` Mattermost-доставка пропускается с info-логом (без записи `failed`); при `true` без webhook поведение прежнее (запись `failed`).
