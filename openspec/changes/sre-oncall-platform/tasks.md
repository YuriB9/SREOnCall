...
- [ ] 3.4 Реализовать обработчик вебхука Zabbix: разбор payload, нормализация в канонический Alert, обработка событий восстановления
- [ ] 3.5 Реализовать middleware идентификации тенанта: вычислить SHA-256 хэш `X-Webhook-Token`, выполнить `HGET oncall:tokens:{hash}` в Redis → получить `tenant_id`; возвращать HTTP 401 при отсутствии совпадения; scheduling обновляет этот Redis-индекс при создании/отзыве токенов
- [ ] 3.6 Реализовать дедупликацию через Redis: SHA-256 fingerprint по отсортированным лейблам + source + tenant_id; SETNX с настраиваемым TTL; удаление ключа для resolved-алерта
- [ ] 3.7 Реализовать AMQP-публикатор на exchange `alerts` (routing key `alert.received`) с повторными попытками и backoff при ошибках
- [ ] 3.8 Написать юнит-тесты для логики нормализации (все три источника), дедупликации и middleware токена
- [ ] 3.9 Написать интеграционный тест: POST на каждый эндпоинт с валидным токеном → проверить публикацию сообщения в RabbitMQ с корректным tenant_id
